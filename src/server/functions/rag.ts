...
import {
	BOOKMARK_AGENT_TOOLS,
	executeBookmarkToolCall,
} from "../ai/bookmarkTools";
...
/**
 * Server Function: Chat with bookmarks using RAG + Agent Function Calling
 */
export const chatWithBookmarks = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }): Promise<RAGChatResult> => {
		...
		const systemPrompt = `你是一个内置于用户个人 AI 工作台（AI Workstation）的高级知识助手与私人外脑。
你的任务是基于用户个人本地 SQLite 收藏库中的书签、工具、文章与素材，回答用户的问题并提供深度洞察。
...
【重要能力与执行规范】:
1. 当用户询问涉及【时间范围】（例如：“我今天/本周/上周/本月/最近7天收藏了哪些网站”、“昨天添加了什么”）、【特定分类/文件夹汇总】（例如：“自媒体分类下有哪些”、“看下设计工具文件夹”）或【数量盘点】时，你必须主动调用 \`query_bookmarks\` 工具从 SQLite 数据库获取最精准的结构化最新数据。
2. 当进行概念探讨或工具推荐时，优先结合给出的参考来源进行总结分析，并用 《书签名称》 明确标识工具。
3. 如果工具查询结果为空，请如实告知用户在该时间段或条件下未检索到相关收藏。
4. 语言要求：排版清晰优美（Markdown 格式，可使用列表、加粗、小标题），专业精炼。

以下是从本地知识库初步语义检索到的相关背景资料（常规参考）：
${contextSnippets}`;

		// 6. Call LLM API (DeepSeek or OpenAI Compatible)
		const apiKey =
			llmConfig.apiKey ||
		...
		try {
			// First Round: Request with tool calling enabled
			...
			if (!firstResponse.ok) {
				...
				// If provider rejects tools schema, gracefully fallback to plain chat completion
				if (
					firstResponse.status === 400 &&
					(errText.includes("tools") || errText.includes("tool_choice"))
				) {
					...
					if (!fallbackRes.ok) {
						throw new Error(`LLM API error: ${await fallbackRes.text()}`);
					}
					const fallbackJson = await fallbackRes.json();
					answer = fallbackJson.choices?.[0]?.message?.content?.trim() || "";
					return {
				...
				throw new Error(
					`LLM API error (${firstResponse.status} ${firstResponse.statusText}): ${errText}`,
				);
			}

			const firstDataJson = await firstResponse.json();
			const firstChoice = firstDataJson.choices?.[0];
			...
			if (message?.tool_calls && message.tool_calls.length > 0) {
				...
				for (const toolCall of message.tool_calls) {
					const toolExecRes = await executeBookmarkToolCall(
						toolCall.id,
						toolCall.function?.name,
						toolCall.function?.arguments,
					);
					toolResults.push(toolExecRes);

					// Replace or prepend references with exact structured search results
					if (toolExecRes.references && toolExecRes.references.length > 0) {
						references = toolExecRes.references;
					}

					llmMessages.push({
						role: "tool",
						tool_call_id: toolCall.id,
						content: toolExecRes.summary,
					});
				}

				// Second Round: Let LLM synthesize final response with tool results
				...
				if (!followUpResponse.ok) {
					...
					throw new Error(
						`LLM Follow-up error (${followUpResponse.status}): ${errText}`,
					);
				}

				const followUpDataJson = await followUpResponse.json();
				answer =
					followUpDataJson.choices?.[0]?.message?.content?.trim() ||
					"";
			} else {
				// No tool called, use the direct generation answer
				answer = message?.content?.trim() || "";
			}
		} catch (error: any) {
			console.error("[chatWithBookmarks] Error:", error);
			throw error;
		}
...
/**
 * Server Function: Generate structured research dossier for a folder
 */
...
请确保语言严谨专业，重点突出，充分利用给定的书签信息。`;
		...
		if (!response.ok) {
			...
			throw new Error(
				`Dossier generation error (${response.status}): ${errText}`,
			);
		}

		const resJson = await response.json();
		const markdown =
			resJson.choices?.[0]?.message?.content?.trim() || "生成综述失败";
