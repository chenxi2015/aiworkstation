...
export function SkillListItem({
	...
	return (
		<div className="relative group rounded-2xl border border-zinc-200/80 dark:border-zinc-800/80 bg-white dark:bg-zinc-900/60 p-4 shadow-2xs hover:shadow-xs hover:border-zinc-300 dark:hover:border-zinc-700 transition-all flex items-center justify-between gap-4">
			{/* Main clickable area */}
			<button
				type="button"
				onClick={() => onOpen(skill)}
				className="flex items-center gap-4 min-w-0 flex-1 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-zinc-400 rounded-xl"
			>
				<SkillIconBadge skill={skill} className="w-12 h-12 shrink-0" />
				<div className="min-w-0 flex-1">
					...
					<p className="mt-1.5 text-[11px] text-zinc-500 dark:text-zinc-400 leading-relaxed truncate">
			...
			<div className="flex items-center gap-4 shrink-0 text-xs text-zinc-400">
				...
				{skill.dirPath && (
					<button
						...
						title="卸载本技能"
						className="p-1.5 rounded-lg text-zinc-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/30 transition-colors z-10"
					>
