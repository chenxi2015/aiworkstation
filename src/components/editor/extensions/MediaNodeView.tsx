...
export const EditorMediaContext = createContext<{ docId: number }>({ docId: 0 });
...
export function MediaNodeView(props: NodeViewProps) {
	...
	const handleDownloadAction = async (e: React.MouseEvent) => {
		...
		let filename = "";
		try {
			filename = new URL(src, window.location.href).pathname.split("/").pop() || "";
		} catch {
			// ignore
		}
		if (!filename) {
	...
	return (
		<NodeViewWrapper
			as="div"
			className={`media-node-view not-prose relative my-3 block max-w-full group select-none transition-all duration-150 rounded-xl ${
				selected
