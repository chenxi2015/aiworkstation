...
import { MousePointerClick, Trash2, Camera, Loader2, Radar, RotateCw } from 'lucide-react';
...
interface GrabTabProps {
  ...
  onRescanSniffedStreams?: () => void;
  isRescanningStreams?: boolean;
  onRefreshTdk: () => void;
...
export const GrabTab: React.FC<GrabTabProps> = ({
  ...
  onRescanSniffedStreams,
  isRescanningStreams = false,
  onRefreshTdk,
  onPushToWorkbench,
}) => {
  ...
  return (
    <div className="p-0 outline-none flex flex-col gap-3">
      ...
      {/* Sniffed HLS video streams on the current page or Rescan Trigger when empty */}
      {sniffedStreams.some((s) => !s.hidden) ? (
        <SniffedStreamsCard
          streams={sniffedStreams}
          onClear={onClearSniffedStreams ?? (() => {})}
          onRescan={onRescanSniffedStreams}
          isRescanning={isRescanningStreams}
        />
      ) : onRescanSniffedStreams ? (
        <div className="rounded-xl border border-dashed border-border/70 bg-surface/50 dark:bg-surface-secondary/30 px-3 py-2.5 flex items-center justify-between gap-2 text-xs animate-in fade-in">
          <div className="flex items-center gap-2 min-w-0 text-muted">
            <Radar className="w-3.5 h-3.5 text-violet-500 shrink-0" />
            <span className="truncate">视频流已清空或未检测到</span>
          </div>
          <button
            type="button"
